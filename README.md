# Ideas4Lego — сайт «Тёма строит»

Стек: **Astro** (генератор сайта) + **Decap CMS** (панель для ребёнка) +
**GitHub Pages** (хостинг). Дизайн — по шаблону в стиле лего-конструктора.

## Разделы сайта и что можно редактировать через /admin

- **Постройки** (`src/content/builds/`) — карточки моделей: фото,
  число деталей, время сборки, сложность, цвет карточки, подробный
  рассказ. У каждой — своя страница `/builds/<адрес>/`.
- **Видео** (`src/content/videos/`) — ссылка на YouTube, превью,
  длительность, просмотры, описание. Своя страница `/videos/<адрес>/`
  со встроенным плеером.
- **Фотогалерея** (`src/content/gallery/`) — просто фото + подпись,
  показывается плиткой на главной.
- **Страницы** (`src/content/pages/`) — произвольные страницы вроде
  «Обо мне»; можно включать/выключать показ в меню и задавать порядок.

Если в разделе ещё нет ни одной записи (например, только что
задеплоили сайт), вместо пустого места показывается фото-заглушка
(цветной градиент) или дружелюбная надпись "пока пусто — добавь в
/admin".

## Что фиксировано в коде (правит профессионал)

- Шапка и подвал сайта (`src/components/Header.astro`,
  `Footer.astro`)
- Главный экран (hero), блок "Для родителей" и форма подписки —
  прямо в `src/pages/index.astro`. Заголовок/текст там просто зашиты
  в JSX-разметку, можно смело переписывать руками.
- Все стили — `src/styles/global.css`. Цветовые токены в `:root`
  наверху файла — самый быстрый способ поменять общий вид сайта.
